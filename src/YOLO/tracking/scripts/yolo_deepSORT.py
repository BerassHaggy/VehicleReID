    ...
    def track_vehicles(self, visible: bool, mot_challenge: bool, write_video: bool):
        ...
        while cap.isOpened():
            ...
            for track in tracks:
                ...
                if confidence == None:
                    confidence = 0
                mot_frame_number = 249 + frame_number
                self.tracking_results.append([mot_frame_number, track_id, int(x1), int(y1), int(w), int(h), confidence, class_id, visibility])
        ...
        motaEvaluator = mt.MOTEvaluator(ground_truth_labels=self.tracking_ground_truth, predictions_filename=self.result_output, results_filename=self.result_output)
        results = motaEvaluator.evaluate()
...
def main():
    ...
    init_signature = inspect.signature(mt.MOTEvaluator.__init__)
    print(init_signature)
    print(mt.__file__)

    mot = mt.MOTEvaluator(ground_truth_labels=tracking_ground_truth, predictions_filename=results_output_filename, results_filename=results_output_filename)
    ...
    deep_sort.track_vehicles(visible=False, mot_challenge=False, write_video=False)
