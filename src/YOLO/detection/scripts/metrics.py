...
class Metric(SimpleClass):
    ...
    def mean_results(self):
        ...
        # include the AP per each class
        ap_per_class = []
        for i in range(len(self.ap)):
            ap_per_class.append(self.ap[i])
        
        """
        print("---------")
        print("AP per class")
        print(ap_per_class)
        print("Number of classes: " + str(len(self.ap)))
        print("Number of classes in AP array: " + str(len(self.all_ap)))
        print("mAP50: " + str(self.map50))
        print("mAP75: " + str(self.map75))
        print("mAP: " + str(self.map))
        print("---------")
        """
        try:
            return [self.mp, self.mr, self.map50, self.map, self.map75, self.ap[0]] + ap_per_class
        except:
            return [self.mp, self.mr, self.map50, self.map, self.map75] + ap_per_class
    ...
    def fitness(self):
        ...
        return (np.array(self.mean_results()[0:4]) * w).sum()
...
class DetMetrics(SimpleClass):
    ...
    def keys(self):
        ...
        # get the individual class names for the AP per class
        try:
            class_ap_keys = [f'metrics/AP-{cls}' for cls in self.names.values()]
        except:
            class_ap_keys = [f'metrics/AP-{cls}' for cls in self.names]
        mean_results = ['metrics/precision(B)', 'metrics/recall(B)', 'metrics/mAP50(B)', 'metrics/mAP50-95(B)', 'metrics/mAP75(B)']
        return mean_results + class_ap_keys
